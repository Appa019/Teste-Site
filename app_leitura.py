...
def obter_cotacao_dolar():
    try:
        ...
        st.warning(f"Não foi possível obter a cotação do dólar. Usando valor padrão.")
        return 5.0
...
def processar_pdf(arquivo_pdf, client):
    try:
...
if uploaded_files and st.session_state.api_key and st.button("🔍 Processar PDFs"):
    ...
    try:
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            ...
            resultado = processar_pdf(uploaded_file, client)
...
# Exibir arquivos processados em formato de paginação
if st.session_state.processed_files:
    st.subheader("📑 Arquivos Processados")
    
    # Configuração da paginação
    arquivos_por_pagina = 10
    total_arquivos = len(st.session_state.processed_files)
    total_paginas = (total_arquivos + arquivos_por_pagina - 1) // arquivos_por_pagina
    ...
    with col2:
        st.write(f"Página {st.session_state.current_page + 1} de {max(1, total_paginas)}")
    
    with col3:
        if st.button("➡️ Próximo") and st.session_state.current_page < total_paginas - 1:
            st.session_state.current_page += 1
    
    # Exibir os PDFs da página atual
    inicio = st.session_state.current_page * arquivos_por_pagina
    fim = min(inicio + arquivos_por_pagina, total_arquivos)
    
    for i in range(inicio, fim):
        arquivo = st.session_state.processed_files[i]
        expander = st.expander(f"{i+1}. {arquivo['nome_arquivo']}")
